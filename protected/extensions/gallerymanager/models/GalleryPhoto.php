...
class GalleryPhoto extends CActiveRecord
{
    ...
    public $galleryDir = 'uploads/gallary';
...
//            array('gallery_id, rank', 'numerical', 'integerOnly' => true),
            ...
            array('file_name', 'length', 'max' => 512),
    ...
    {
        return Yii::app()->request->baseUrl . '/' . $this->galleryDir . '/' . $this->getFileName('') . '.' . $this->galleryExt;
    }

    private function getFileName($version = '')
    {
        return $this->getImageDir() . '/' . $this->id;
    ...
    {
        $d = Yii::getPathOfAlias('application') . '/../' . $this->galleryDir . '/';
        $p = $d . $this->getImageDir();
        
        if (!is_dir($p)) {
            @mkdir($p, 0777, true);
            //@chmod($p, 0777);
        }
        $p = $d . $this->getFileName('') ;
        //save image in original size
        Yii::app()->image->load($path)->save($p . '.' . $this->galleryExt);
        @chmod($p . '.' . $this->galleryExt, 0777);
...
//        }
    ...
    {
        $p = Yii::getPathOfAlias('application') . '/../' . $fileName;
...
//        }
    ...
    public static function getPhotoUrl($photo, $type = 'small')
    {
        if ($photo->type > 0) {
            return $photo->file_name;
        }
        return 'http://' . $_SERVER['SERVER_NAME'] . $photo->getUrl();
    }
